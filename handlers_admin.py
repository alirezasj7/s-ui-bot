...
def create_admin_router(db, is_main_bot: bool = True, bot_manager=None) -> Router:
    ...
    async def process_panel_server_url(message: Message, state: FSMContext):
        ...
        data = await state.get_data()
        if data.get("panel_type") in ("3xui", "sui"):
            # 3X-UI و S-UI فقط با API Token احراز هویت می‌شوند و یوزرنیم ندارند؛
            ...
            await state.update_data(username=panel_type)
            await state.set_state(AdminAddPanelServer.waiting_password)
            if panel_type == "sui":
                await message.answer(
                    "API Token پنل S-UI را بفرست (نه پسورد ادمین!):\n"
                    "از داخل S-UI به Admin ← API Tokens برو، یک توکن بساز و همان را اینجا بفرست."
                )
            else:
                await message.answer(

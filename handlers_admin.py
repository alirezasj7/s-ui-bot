...
def create_admin_router(db, is_main_bot: bool = True, bot_manager=None) -> Router:
    ...
    def panel_server_readiness_text(server) -> str:
        ...
        return "⚠️ inbound و آدرس Subscription هنوز تنظیم نشده"
    ...
    async def cb_panel_server_type_selected(call: CallbackQuery, state: FSMContext):
        ...
        if panel_type != "sui":
            await call.answer("فقط S-UI-X پشتیبانی می‌شود.", show_alert=True)
            return
    ...
    async def process_panel_server_url(message: Message, state: FSMContext):
        ...
        data = await state.get_data()
        if data.get("panel_type") != "sui":
            await state.clear()
            await message.answer("فقط S-UI-X پشتیبانی می‌شود؛ دوباره از ابتدا تلاش کنید.")
            return
        await state.update_data(username="sui")
        await state.set_state(AdminAddPanelServer.waiting_password)
        await message.answer(
            "API Token پنل S-UI-X را بفرست (نه پسورد ادمین):\n"
            "در S-UI-X از Settings → API Tokens یک توکن با دسترسی admin بساز."
        )
    ...
    async def process_panel_server_username(message: Message, state: FSMContext):
        ...
        await state.set_state(AdminAddPanelServer.waiting_password)
        data = await state.get_data()
        await message.answer("API Token پنل S-UI-X را بفرست:")
    ...
    async def process_panel_server_template_user(message: Message, state: FSMContext):
        await state.clear()
        await message.answer("S-UI-X از قالب کاربر استفاده نمی‌کند؛ یک inbound و آدرس Subscription تنظیم کنید.")
    ...
    async def process_panel_server_template_update(message: Message, state: FSMContext):
        ...
        if not server:
            ...
            return
        await state.clear()
        await message.answer("S-UI-X از قالب جداگانه استفاده نمی‌کند؛ کاربر نمونه داخل خود پنل نگه‌داری می‌شود.")

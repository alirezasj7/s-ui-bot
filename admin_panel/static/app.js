...
/* ============================================================== panels === */
const PANEL_TYPE_OPTIONS_HTML = `<option value="sui">S-UI-X (sing-box)</option>`;
const PANEL_INBOUND_SELECT_TYPES = ['sui'];
const PANEL_SUB_BASE_URL_TYPES = ['sui'];
const PANEL_TEMPLATE_BASED_TYPES = [];
...
function wirePanelAddForm(body, close) {
  ...
  function syncTypeUI() {
    ...
    if (PANEL_INBOUND_SELECT_TYPES.includes(t)) {
      userInp.style.display = 'none'; userInp.value = '';
      passInp.placeholder = 'API Token (Settings → API Tokens)';
      hint.style.display = 'block';
      hint.textContent = 'S-UI-X فقط با API Token کار می‌کند؛ یوزرنیم لازم نیست. بعد از ثبت باید inbound و لینک Subscription را هم تنظیم کنی.';
    } else if (false) {
      userInp.style.display = ''; userInp.placeholder = 'هر مقداری (استفاده نمی‌شود)';
      passInp.placeholder = 'API Token S-UI-X';
      hint.style.display = 'block';
      hint.textContent = 'فقط S-UI-X پشتیبانی می‌شود.';
    } else {
  ...
  $('#p-save', body).addEventListener('click', async () => {
    ...
    if (!passInp.value.trim()) return toast(PANEL_INBOUND_SELECT_TYPES.includes(panel_type) ? 'API Token الزامی است.' : 'رمز/کلید الزامی است.', true);

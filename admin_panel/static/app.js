...
const PANEL_TYPE_OPTIONS_HTML = [
  ['pasarguard', 'PasarGuard'], ['3xui', '3X-UI'], ['sui', 'S-UI'], ['marzban', 'Marzban'],
  ['marzneshin', 'Marzneshin'], ['hiddify', 'Hiddify'],
].map(([v, l]) => `<option value="${v}">${l}</option>`).join('');
const PANEL_INBOUND_SELECT_TYPES = ['3xui', 'sui'];
const PANEL_SUB_BASE_URL_TYPES = ['3xui', 'sui', 'hiddify'];
...
function wirePanelAddForm(body, close) {
  ...
  function syncTypeUI() {
    ...
    if (PANEL_INBOUND_SELECT_TYPES.includes(t)) {
      ...
      passInp.placeholder = t === 'sui' ? 'API Token (Admin ← API Tokens)' : 'API Token (Settings ← Security در پنل)';
      hint.style.display = 'block';
      hint.textContent = `${t === 'sui' ? 'S-UI' : '3X-UI'} فقط با API Token کار می‌کند؛ یوزرنیم لازم نیست. بعد از ثبت باید inbound و لینک Subscription را هم تنظیم کنی.`;
    } else if (t === 'hiddify') {

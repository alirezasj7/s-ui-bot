...
async function renderAdminPanelsSection() {
  ...
  try {
    if (adminPanelsView.level === "add-server") {
      ...
      const NO_TEMPLATE_TYPES = ["3xui", "sui", "hiddify"];
      const syncPsType = () => {
        ...
        document.getElementById("ps-xui-hint").style.display = ["3xui", "sui"].includes(psType.value) ? "block" : "none";
      ...
      document.getElementById("ps-save").onclick = async () => {
        ...
        if (!payload.name || !payload.api_url || !payload.api_password ||
            (!["3xui", "sui"].includes(panelType) && !payload.api_username)) {
          errBox.textContent = "نام، آدرس و اطلاعات احراز هویت الزامی هستند."; return;
        }
        if (!["3xui", "sui", "hiddify"].includes(panelType) && !payload.template_username) {
        ...
        try {
          ...
          tg.HapticFeedback.notificationOccurred("success");
          if (["3xui", "sui"].includes(panelType)) {
            adminPanelsView = { level: "xui-config", serverId: res.id, inbounds: res.inbounds, name: payload.name, panelType };
    ...
    if (adminPanelsView.level === "xui-config") {
      ...
      document.getElementById("ps-xui-save").onclick = async () => {
        ...
        try {
          ...
          tg.HapticFeedback.notificationOccurred("success");
          notify(`سرور ${adminPanelsView.panelType === "sui" ? "S-UI" : "3X-UI"} با موفقیت تنظیم شد.`);
          adminPanelsView = { level: "servers" };
          renderAdminPanelsSection();
        } catch (e) {
          errBox.textContent = e.message;
        ...
        <div style="display:flex;flex-direction:column;gap:8px;margin-bottom:12px">
          ${servers.map((s) => `
            <div style="background:var(--glass-bg);border:1px solid var(--glass-brd);border-radius:10px;padding:10px 12px">
              ...
              <div class="hint-text" style="margin:0 0 4px">${["3xui", "sui"].includes(s.panel_type)
              ...
              <div style="display:flex;gap:6px;flex-wrap:wrap">
                ...
                ${["3xui", "sui"].includes(s.panel_type)
      ...
      btn.onclick = async () => {
        ...
        try {
          ...
          adminPanelsView = {
            ...
            panelType: server && server.panel_type,

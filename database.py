...
class Database:
        ...
        with self._get_conn() as conn:
            ...
            c.executescript(
                ...
                CREATE TABLE IF NOT EXISTS panel_servers (
                    id INTEGER PRIMARY KEY AUTOINCREMENT,
                    name TEXT NOT NULL,
                    panel_type TEXT NOT NULL DEFAULT 'sui',
    ...
    def _migrate_columns(self, conn):
        migrations = [
            ...
            # چند-inbound برای S-UI-X: یک سرور می‌تواند همزمان چند
        ...
        # This release is S-UI-X only.  Preserve legacy records for audit, but
        # deactivate them so they cannot be selected or used for provisioning.
        if self._column_exists(conn, "panel_servers", "panel_type"):
            conn.execute("UPDATE panel_servers SET is_active=0 WHERE panel_type <> 'sui'")
    ...
    def add_panel_server(self, name: str, panel_type: str, api_url: str,
        ...
        if panel_type != "sui":
            raise ValueError("Only S-UI-X panel servers are supported")
        with self._get_conn() as conn:
            cur = conn.execute(
    ...
    def get_panel_servers(self, active_only: bool = False):
        with self._get_conn() as conn:
            q = "SELECT * FROM panel_servers WHERE panel_type='sui'"
            if active_only:
                q += " AND is_active=1"
            q += " ORDER BY id"
            return conn.execute(q).fetchall()
